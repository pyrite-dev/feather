...
static int directive(fr_context_t* context, int argc, char** argv) {
	if(strcmp(argv[0], "LoadModule") == 0) {
		if(argc == 3) {
			...
			void*	     h	 = ppr_dlopen(p); /* XXX: this leaks! but we don't know good ways to solve... yet */
			fr_module_t* mod = ppr_dlsym(h, argv[1]);
			char*	     s;

			if(mod == NULL) {
				s   = ppr_strvacat("_", argv[1], NULL); /* watcom likes to export this way */
				mod = ppr_dlsym(h, s);
				free(s);
			}
