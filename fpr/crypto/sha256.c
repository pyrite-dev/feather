...
const fpr_uint32_t sha256_initial_h[8] = {0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
					  0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19};

const fpr_uint32_t sha256_round_k[64] = {0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1,
		...
		if(cursor * 64 + 64 <= len) {
			for(j = 0; j < 16; j++) {
				w[j] = sha256_endian_read32(
				    (fpr_uint8_t*)data + cursor * 64 + j * 4);
			}
		} else {
			if(cursor * 64 < len) {
			...
			for(j = 0; j < 16; j++) {
				w[j] = sha256_endian_read32((fpr_uint8_t*)&w[j]);
			}
		}

		for(j = 16; j < 64; j++) {
			fpr_uint32_t s1 = sha256_ror(w[j - 2], 17) ^ sha256_ror(w[j - 2], 19) ^ (w[j - 2] >> 10);
			fpr_uint32_t s0 = sha256_ror(w[j - 15], 7) ^ sha256_ror(w[j - 15], 18) ^ (w[j - 15] >> 3);
			w[j]		= s1 + w[j - 7] + s0 + w[j - 16];
		}

		for(j = 0; j < 64; j++) {
			fpr_uint32_t ch	 = (t[4] & t[5]) ^ (~t[4] & t[6]);
			fpr_uint32_t maj = (t[0] & t[1]) ^ (t[0] & t[2]) ^ (t[1] & t[2]);
			fpr_uint32_t S0	 = sha256_ror(t[0], 2) ^ sha256_ror(t[0], 13) ^ sha256_ror(t[0], 22);
			fpr_uint32_t S1	 = sha256_ror(t[4], 6) ^ sha256_ror(t[4], 11) ^ sha256_ror(t[4], 25);

			fpr_uint32_t t1 = t[7] + S1 + ch + sha256_round_k[j] + w[j];
			fpr_uint32_t t2 = S0 + maj;
